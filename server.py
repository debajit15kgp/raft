...
class KeyValueStore(raft_pb2_grpc.KeyValueStoreServicer):
    ...
    def create_channel_with_source_port(self, target_id):
        ...
        sock.bind(('', BASE_SOURCE_PORT + self.id*5 + target_id))
        channel_options = [
    ...
    def GetState(self, request, context):
        return raft_pb2.State(term=self.currentTerm, isLeader=(self.leaderId==self.id))
    ...
    def Put(self, request, context):
        if self.role == "leader":
            self.log.append({"term": self.currentTerm,
                            ...
                            "value": request.value})
            self.write_persistent_memory()
            res =  raft_pb2.GenericResponse(success=True)
            # print(str(res), "hansin")
            return res
